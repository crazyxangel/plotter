...
ser = serial.Serial("COM5",115200,timeout = 0) #starting up the serial communication
    ...
    try:
        if z == 0:                              # X
            ...
            if x < 5:                                                                                           #first coordinate cant be checked by previous one
                ...
                x_steps.append(int(round(((float(values[x])/mmstep)*factor))))                                  #appends the step value
                ...
                x_steps.append(int(round(((float(values[x])/mmstep)*factor))))
        elif z == 1:                            # Y
            z +=1
            if x < 5: 
                y_coords.append(float(values[x]))
                y_steps.append(int(round(((float(values[x])/mmstep)*factor))))
